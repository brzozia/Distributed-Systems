    ...
    public void connect(String pesel, CitizenPrx proxy, com.zeroc.Ice.Current current) {
        ...
        if(!map.containsKey(pesel)) {
            ...
            if (map.get(pesel).getResponses() != null) {
                for (int i = 0; i < map.get(pesel).getResponses().size(); i++) {
                    try {
                        map.get(pesel).getProxy().handleResponse(map.get(pesel).getResponses().get(i));
                        map.get(pesel).getResponses().remove(i);
                        System.out.println("Overdue response send to " + pesel);
                    } catch (ConnectionLostException e) {
                        System.out.println("Connection Lost Exception occurred while sending to " + pesel);
                        break;
                    }
                }
