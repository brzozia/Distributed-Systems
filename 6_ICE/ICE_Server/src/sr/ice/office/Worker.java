...
public class Worker implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            System.out.println("["+requestType+ " "+ requestId+"] "+"Response send to "+permission.person.pesel);
        }
        catch(ConnectionLostException e){
            synchronized (map) {
                map.get(permission.person.pesel).addResponse(result);
            }
            System.out.println("["+requestType+ " "+ requestId+"] "+"Connection Lost Exception occurred while sending to "+permission.person.pesel);
        }
    }
